...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def assert_extra_args(
        ...
        if self.model_type != "chroma":
            ...
            assert args.apply_t5_attn_mask, "apply_t5_attn_mask must be True for Chroma"
        ...
        if args.split_mode:
            if args.blocks_to_swap is not None:
                logger.warning("split_mode is deprecated. Because `--blocks_to_swap` is set, `--split_mode` is ignored.")
            else:
                logger.warning("split_mode is deprecated. Please use `--blocks_to_swap` instead. `--blocks_to_swap 18` is automatically set.")
    ...
    def load_target_model(self, args, weight_dtype, accelerator):
        ...
        if self.use_clip_l:
            ...
            clip_l = flux_utils.dummy_clip_l()
        clip_l.eval()
    ...
    def cache_text_encoder_outputs_if_needed(
        ...
        if args.cache_text_encoder_outputs:
            if not args.lowram:
                ...
                vae.to("cpu")
                # VULCAN CRITICAL FIX: Disable UNet offload to avoid "Cannot copy out of meta tensor" error
                # unet.to("cpu") 
            ...
            if not args.lowram:
                ...
                # unet is already on original device (Meta)
    ...
    def prepare_unet_with_accelerator(self, args, accelerator, unet):
        # VULCAN CRITICAL FIX: Force gradients on inputs for NF4/Quantized models
        if args.gradient_checkpointing:
             logger.info("VULCAN: Enabling gradient checkpointing and input gradients for Flux LoRA training.")
             unet.train()
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    # Conflict removed: --split_mode is now handled by flux_train_utils
    return parser
